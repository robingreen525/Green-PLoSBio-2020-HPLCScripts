...
files<-list.files(path='20171030_exportdataq/standards/')

for(file in files)
{
  print(file)
  uMGSH<-as.numeric(strsplit(file,split = 'uMGSH')[[1]][1])
  ...
  if(uMGSH>0)
  {
    ...
    tryCatch({
            ...
            r<-read.table(paste('20171030_exportdataq/standards/',file,sep='/'),header=T,sep='\t',skip=26)
            ...
            uM.GSH<-rep(uMGSH,times=nrow(r))
            timerep<-rep(timerep,times=nrow(r))
            #timerep<-rep(timerep,time=nrow(r))
            minutesIncubation<-rep(20,times=nrow(r))
            r<-cbind(r,sample,uM.GSH,minutesIncubation,timerep)
...
g<-ggplot(data=Spectra_Short,aes(x=RT,y=CorrectedIntense,col=timerep))+
  geom_point()+facet_grid(uM.GSH ~ timerep)
print(g)
ggsave(g,filename = 'Figures/HPLC_StandardTraces.png')

mean_areas<-data.frame()
#Spectra_Short<-Spectra_Short[which(Spectra_Short$timerep>1),]
...
for(samp in unique(Spectra_Short$sample))
{
  ...
  df<-data.frame(samp,avg,sd,t$uM.GSH[1],t$timerep[1])
  colnames(df)<-c('Sample','Average_Area','SD_Area','uMGSH','timerep')
  mean_areas<-rbind(mean_areas,df)
}

write.csv(mean_areas,file = 'stdcurve.csv')
